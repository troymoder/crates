...
use crate::error::{OpenApiError, ServiceError};
use crate::types::{
...
impl GeneratedMethod {
    ...
    fn new(
    ...
    ) -> Result<GeneratedMethod, ServiceError> {
        ...
        let input_path = match &method.input {
            ...
            ProtoServiceMethodIo::Stream(_) => return Err(ServiceError::StreamingNotSupported),
        };
...
pub(super) fn handle_service(
    ...
    let json_openapi = openapi
        .to_json()
        .map_err(|e| ServiceError::OpenApiSchema(OpenApiError::Json(e)))?;
